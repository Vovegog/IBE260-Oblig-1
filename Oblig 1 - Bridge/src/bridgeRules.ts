...
import { rank, errorHandler, bidding } from "./main";
...
export function checkValidBid(cardToCheck: [string, rank]): boolean {
    try {
        let valid = false;
        ...
        const playedCardSuit: Suit = cardToCheck[0] as Suit;   // Convert the suit of the played card to a number
        const playedCardValue: Rank = cardToCheck[1] as Rank;  // Convert the rank of the played card to a number
